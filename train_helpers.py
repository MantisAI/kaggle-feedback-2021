...
def train_epoch(
    epoch, model, optimizer, scaler, training_loader, config, compute_accuracy=True
):
    tr_loss, tr_accuracy = 0, 0
    nb_tr_examples, nb_tr_steps = 0, 0
    loss_ftc = None
    if config["loss_weights"] is not None:
        loss_ftc = CrossEntropyLoss(
            weight=torch.tensor(config["loss_weights"]).to(config["device"])
        )
