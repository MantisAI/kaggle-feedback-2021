...
def get_loaders(
    ...
    train_idx = np.random.choice(
        np.arange(len(IDS)), int(config["train_val_split"] * len(IDS)), replace=False
    )
    ...
    train_dataset = data.loc[
        data["id"].isin(IDS[train_idx]), ["text", "entities"]
    ].reset_index(drop=True)
    test_dataset = data.loc[data["id"].isin(IDS[valid_idx])].reset_index(drop=True)
    training_set = dataset(
        train_dataset, tokenizer, config["max_length"], False, labels_to_ids
    )
    testing_set = dataset(
        test_dataset, tokenizer, config["max_length"], True, labels_to_ids
    )
    ...
    # TEST DATASET
    test_texts_set = dataset(
        test_texts, tokenizer, config["max_length"], True, labels_to_ids
    )
